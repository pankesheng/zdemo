...
<script>
    ...
    //添加
	function add(){
		z_openIframe('新增', 700, 400, '${contextPath}/user4/toadd.do');
		// window.location.href = '${contextPath}/user4/toadd.do';
	}
	
	//编辑
	function editItem(id){
		z_openIframe('编辑', 700, 400, '${contextPath}/user4/tomodify/' + id + '.do');
		// window.location.href = '${contextPath}/user4/tomodify/' + id + '.do';
	}

	//删除
	function removeItems(){
		var data = grid.getSelectedDataString('id');
		z_delete(data, '${contextPath}/user4/delete.ajax');
	}
